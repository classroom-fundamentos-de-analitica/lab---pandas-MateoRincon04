...
def pregunta_08():
    ...
    """
    result = tbl0.copy()
    result["suma"] = result._c0 + result._c2
    return result
...
def pregunta_10():
    ...
    """
    tb = tbl0
    result = tb.groupby('_c1').agg({'_c2': lambda x: sorted(list(x))})
...
def pregunta_11():
    ...
    """
    tb = tbl1
    result = tb.groupby('_c0').agg({'_c4': lambda x: sorted(list(x))})

    for _, row in result.iterrows():
        row['_c4'] = ",".join([str(int) for int in row['_c4']])

    result.insert(0, '_c0', range(0, 40))
    return result
...
def pregunta_12():
    ...
    result = tl.groupby('_c0').agg({'_c5': lambda x: sorted(x)})
